...
my_parser.add_argument('-s','--stats', help='stats nii image should be e. g. f statistic map', required=True)
...
my_parser.add_argument('-thr', '--pvalue_thr', type=float, help="threshold for binarizing p value", default=0.05)
my_parser.add_argument('-exp', '--experiment', type=str, help="experiment dir either cold or walking", default='walking')
...
print("shape pvalue: ", pvals_img_arr.shape,
      "pvalue min: ", pvals_img_arr.min(),
      " pvalue max: ", pvals_img_arr.max())

# binarize pval
thr = -np.log10(thr)
pvals_img_arr_bin = np.zeros_like(pvals_img_arr, dtype='bool')
pvals_img_arr_bin[pvals_img_arr > thr] = True
nib.save(nib.Nifti1Image(pvals_img_arr_bin.astype(np.uint8), pvals_img_nii.affine, pvals_img_nii.header),
         os.path.join(out_dir, 'p_values_bin.nii.gz'))


# dilate binarized pval
pvals_img_arr_bin_d = scp.binary_dilation(pvals_img_arr_bin, iterations=1).astype(np.uint8)
...
labeled_pvals, num_clusters_sig = scp.label(pvals_img_arr_bin_d.astype(np.uint8), structure=s)
...
cluster_props = measure.regionprops_table(label_image=labeled_pvals,
...
prop_list = ('intensity_mean',
            'intensity_max',
            'intensity_min')
cluster_props_temp = measure.regionprops_table(label_image=labeled_pvals,
                                          intensity_image=pvals_img_arr,
                                          properties=prop_list)
...
for i in range(len(data['coords'])):
    ...
    for img_ndarray in grp_treated:
        intensity_mean_grp_treated.append(np.mean([img_ndarray[tuple(coord)] for coord in cluster_coords]))

    data['grp_treated_mean_intensity'][i] = intensity_mean_grp_treated

    intensity_mean_grp_ctrl = []
    for img_ndarray in grp_ctrl:
        intensity_mean_grp_ctrl.append(np.mean([img_ndarray[tuple(coord)] for coord in cluster_coords]))
...
for i in range(len(treated_values)):
    for j in range(i):
        ...
        res = pearsonr(np.concatenate((treated_values[i], ctrl_values[i])), np.concatenate((treated_values[j], ctrl_values[j])), method=method)     
        method = ss.BootstrapMethod(method='BCa', random_state=rng)
        low, high = res.confidence_interval(confidence_level=0.8, method=method)
        print(i, j, res.statistic, low, high, res.pvalue, treated_values[i], treated_values[j])
        corr_matrix[i, j] = low
...
correlation_matrix_df = pd.DataFrame(corr_matrix, index=data.index[:N_thr], columns=data.index[:N_thr])
pvalue_matrix_df = pd.DataFrame(pvalue_matrix, index=data.index[:N_thr], columns=data.index[:N_thr])
...
distance_matrix_df = pd.DataFrame(distance_matrix, index=data.index[:N_thr], columns=data.index[:N_thr])
# print(distance_df)
...
significant_correlation_matrix = np.where(significant_mask, correlation_matrix_df, np.nan)
significant_correlation_matrix_df = pd.DataFrame(significant_correlation_matrix, index=index, columns=columns)

significant_distance_matrix = np.where(significant_mask, distance_matrix_df, np.nan)
significant_distance_matrix_df = pd.DataFrame(significant_distance_matrix, index=index, columns=columns)
...
sns.heatmap(significant_correlation_matrix_df, cmap="coolwarm", linewidth=.5, annot=True,annot_kws={"fontsize":4, "fontweight":"bold"}, cbar=True, ax=axes[0], xticklabels = True, yticklabels= True)
axes[0].set_title('Correlation Coefficient')

# Plot p-values with heatmap
sns.heatmap(pvalue_matrix_df, cmap="RdBu_r", annot=True, linewidth=.5, annot_kws={"fontsize":4, "fontweight":"bold"}, cbar=True, ax=axes[1], xticklabels = True, yticklabels= True)
...
sns.heatmap(significant_distance_matrix_df,  linewidth=.5, cmap="Blues",cbar=True, ax=axes[2],  xticklabels = True, yticklabels= True)
...
# Plot correlation matrix with heatmap
sns.heatmap(significant_correlation_matrix_df, mask=mask1, square = True,  cmap="coolwarm", linecolor='gray', linewidth=.5, annot=True,annot_kws={"fontsize":5, "fontweight":"bold"}, cbar=True, ax=ax, xticklabels = True, yticklabels= True)

# Plot distance matrix with heatmap
# sns.heatmap(significant_distance_matrix_df, cmap="Blues", annot=True, annot_kws={"fontsize":4}, cbar=True, ax=axes[2],  xticklabels = True, yticklabels= True)
sns.heatmap(significant_distance_matrix_df,mask=mask2, square = True, linewidth=.5, annot=True, annot_kws={"fontsize":5, "fontweight":"bold"}, linecolor='gray', cmap=sns.cubehelix_palette(as_cmap=True),cbar=True, ax=ax,  xticklabels = True, yticklabels= True, fmt='.1f')

# Adjust layout to prevent overlap
plt.tight_layout()

# Save or show the plot
plt.savefig(os.path.join(out_dir,'corr_analysis_sig_combined_effect_size_edited_alpha05.png'), dpi = 600)
# plt.show()
...
# Save or show the plot
plt.savefig(os.path.join(out_dir,'corr_analysis_sig_combined_effect_size_edited_alpha01.png'), dpi = 600)