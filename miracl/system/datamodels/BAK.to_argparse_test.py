...
def create_parser(modules, module_type: str):
    for name, obj in modules.__dict__.items():
        if isinstance(obj, MiraclObj):
            if should_include_in_cli(obj, module_type):
                ...
                cli_group_info = None
                if obj.flow.get(module_type, {}).get("cli_group") is not None:
                    cli_group_info = obj.flow[module_type][
                        "cli_group"
...
create_parser(objs, "ace")
create_parser(objs2, "ace")
