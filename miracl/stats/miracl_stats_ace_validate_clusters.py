...
def parsefn():
    ...
    parser.add_argument(
        "-c",
        "--control",
        help="path to control group; should have directories for each subject;each directory contains registration folder and 3D seg file from ACE",
        required=True, # TODO: fix
    )
    parser.add_argument(
        "-t",
        "--treated",
        help="path to treated group; specs should be similar to control group",
        required=True, # TODO: fix
    )
    parser.add_argument(
        "-o",
...
def binarize_dialte_clusters(
    ...
    nib.save(
        nib.Nifti1Image(pvals_img_arr_bin, pvals_img_nii.affine, pvals_img_nii.header),
        pval_bin,
    )
...
def warp_sig_clusters(
    ...
    warp_cmd = (
        "/data2/projects/Ahmadreza/ACE_reviewer_response_experiments/stats/validate_clusters/miracl_lbls_warp_to_clar_space_v4.sh -r %s -l %s -o %s -c %s -d %s -f %s -i %s -t %s"
        % (
            reg_dir,
            dil_bin_pval_comp_fil,
            ort_file,
...
def compute_region_props(
    ...
    if mode == "atlas":
        ...
        cluster_props = measure.regionprops_table(
            label_image=labeled_pval, intensity_image=mean_diff, properties=prop_list
        )
        ...
        cluster_props_temp = measure.regionprops_table(
            label_image=labeled_pval, intensity_image=pval, properties=prop_list
        )
        ...
        prop_list = ("intensity_mean", "intensity_max", "intensity_min")
        cluster_props_temp = measure.regionprops_table(
            label_image=labeled_pval, intensity_image=stats, properties=prop_list
        )
...
def summarize_neuron_count_to_csv(
    neuron_json, subj, out_dir, sig_clusters_summary_csv_path
):
    ...
    for i, cluster in enumerate(clusters):
        ...
        sig_clusters_summary_csv.iloc[
            i, sig_clusters_summary_csv.columns.get_loc(f"{subj}_bbox_area_native")
...
def neuron_count_stats(
    sig_clusters_summary_csv_path,
    treated_group_subj_name,
    ctrl_group_subj_name,
    out_dir,
):
    sig_clusters_summary_csv = pd.read_csv(sig_clusters_summary_csv_path, index_col=0)
    clusters = sig_clusters_summary_csv["label"]
    ...
    sig_clusters_summary_csv.to_csv(
        os.path.join(out_dir, "sig_clusters_summary_with_stats.csv")
    )
...
def main():
    ...
    grp_treated_subj_list_paths = [
        os.path.join(treated_dir, subj) for subj in os.listdir(treated_dir)
    ]
    grp_ctrl_subj_list_paths = [
        os.path.join(ctrl_dir, subj) for subj in os.listdir(ctrl_dir)
    ]
    ...
    grp_treated_subj_list_reg_paths = [
        os.path.join(subj, "registration", "clar_allen_reg")
        for subj in grp_treated_subj_list_paths
    ]
    ...
    grp_ctrl_subj_list_reg_paths = [
        os.path.join(subj, "registration", "clar_allen_reg")
        for subj in grp_ctrl_subj_list_paths
    ]
    ...
    grp_treated_subj_list_reg_orientation_paths = [
        os.path.join(subj, "registration", "orientation.txt")
        for subj in grp_treated_subj_list_paths
    ]

    # grp_ctrl_subj_list_reg_orientation_paths = [os.path.join(subj,"orientation.txt") for subj in grp_ctrl_subj_list_paths]
    grp_ctrl_subj_list_reg_orientation_paths = [
        os.path.join(subj, "registration", "orientation.txt")
        for subj in grp_ctrl_subj_list_paths
    ]
    ...
    grp_treated_subj_list_seg_file_paths = [
        os.path.join(subj, "ace_unetr_mc_out.tif")
        for subj in grp_treated_subj_list_paths
    ]

    # grp_ctrl_subj_list_seg_file_paths = [os.path.join(subj, "ace_out.tif") for subj in grp_ctrl_subj_list_paths]
    grp_ctrl_subj_list_seg_file_paths = [
        os.path.join(subj, "ace_unetr_mc_out.tif") for subj in grp_ctrl_subj_list_paths
    ]
    ...
    dil_bin_pval_comp_fil = os.path.join(
        out_dir, "p_values_bin_dilated_conncomp_filtered.nii.gz"
    )

    pvals_img_arr, labeled_pvals = binarize_dialte_clusters(
        p_value,
        pvalue_thr,
        out_dir,
        dil_bin_pval,
    ...
    for idx, subj in enumerate(grp_ctrl_subj_list_paths):
        # get the size of the raw image from segmentation file
        memmap_seg_file_shape = tifffile.memmap(
            grp_ctrl_subj_list_seg_file_paths[idx]
        ).shape
        org_clar = f"{memmap_seg_file_shape[0]}x{memmap_seg_file_shape[1]}x{memmap_seg_file_shape[2]}"

        warped_cluster = f"p_values_bin_dilated_conncomp_filtered_warped_ctrl_{os.path.basename(subj)}"
        warp_sig_clusters(
            grp_ctrl_subj_list_reg_paths[idx],
            dil_bin_pval_comp_fil,
    ...
    for idx, subj in enumerate(grp_treated_subj_list_paths):
        ...
        warped_cluster_dir = os.path.join(
            out_dir,
            f"p_values_bin_dilated_conncomp_filtered_warped_treated_{os.path.basename(subj)}_tiff_clar",
        )
    ...
    for idx, subj in enumerate(grp_treated_subj_list_paths):
        summarize_neuron_count_to_csv(
            os.path.join(
                out_dir + "/" + os.path.basename(subj), "neuron_info_with_label.json"
            ),
