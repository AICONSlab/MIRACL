...
from miracl.system.objs.objs_stats.objs_tfce.objs_tfce import TFCE
from miracl.api.enums import ModuleType
from miracl.system.registry.registry_refactor.parsed_args_flag_resolver import (
    ParserUtilFns,
)
    ...
    if adj.shape[0] > thr:
        ...
        if data.max() > eps:
            ...
            f_obs, clusters, cluster_pv, H0 = spatio_temporal_cluster_test(
                [
                    thresh_data[0 : len(control_imgs),],
...
def main():
    ...
    TFCECLIIntrospector = RegistryIntrospector(registry)
    module_objects = TFCECLIIntrospector.get_modules_as_dict()

    tfce_cli_builder_parser_input = MiraclObjSerializer.serialize_for_cli(
        module_objects,
        module_parser=True,
    )
    tfce_cli_builder = MiraclCLIBuilder()
    _ = tfce_cli_builder.build_parser(tfce_cli_builder_parser_input)
    parsed_args = tfce_cli_builder.parse()

    print(parsed_args)
    print(TFCE.control_input_dir.resolve(ModuleType.MODULE)["cli_l_flag"])
    print(
        getattr(
            parsed_args, TFCE.control_input_dir.resolve(ModuleType.MODULE)["cli_l_flag"]
        )
    )

    control_dir = ParserUtilFns.resolve_flag_value(parsed_args, TFCE.control_input_dir)
    treated_dir = ParserUtilFns.resolve_flag_value(parsed_args, TFCE.treated_input_dir)
    out_dir = ParserUtilFns.resolve_flag_value(parsed_args, TFCE.output_dir)
    num_perm = ParserUtilFns.resolve_flag_value(parsed_args, TFCE.number_permutations)
    atl_dir = ParserUtilFns.resolve_flag_value(parsed_args, TFCE.atlas_directory)
    img_res = ParserUtilFns.resolve_flag_value(parsed_args, TFCE.voxel_size)
    smoothing_fwhm = ParserUtilFns.resolve_flag_value(parsed_args, TFCE.smoothing_fwhm)
    tfce_start = ParserUtilFns.resolve_flag_value(parsed_args, TFCE.start)
    tfce_step = ParserUtilFns.resolve_flag_value(parsed_args, TFCE.step)
    tfce_h = ParserUtilFns.resolve_flag_value(parsed_args, TFCE.h_power)
    tfce_e = ParserUtilFns.resolve_flag_value(parsed_args, TFCE.e_power)
    cpuload = ParserUtilFns.resolve_flag_value(parsed_args, TFCE.cpu_load)
    stp = ParserUtilFns.resolve_flag_value(parsed_args, TFCE.step_down_p)
    mask_thr = ParserUtilFns.resolve_flag_value(parsed_args, TFCE.mask_thr)
    hemi = ParserUtilFns.resolve_flag_value(parsed_args, TFCE.hemi)
    side = ParserUtilFns.resolve_flag_value(parsed_args, TFCE.side)
    ...
    if len(control_dir) > 1:
        ...
        control_imgs = [
            str(file.relative_to(control_base_dir)) for file in control_imgs
        ]
