...
ORIENTATION_DICT = {0: "x", 1: "y", 2: "z"}


def parsefn():
    parser = argparse.ArgumentParser(
        usage="%(prog)s -f fixed_file -r registered_file [-o output_file] [-s segmentation_file] [-sl slices] [-sc scale] [-c] [-cr min max] \n\n"
    ...
    required.add_argument(
        ...
        type=str,
        metavar="",
        help="fixed image used in registration",
        required=True,
    )
    required.add_argument(
        "-r", "--reg", type=str, metavar="", help="registration output", required=True
    )
    ...
    optional.add_argument(
        "-sl", "--slices", type=int, metavar="", help="number of slices", default=7
    )
    optional.add_argument(
        "-sc",
        "--scale",
        type=int,
        metavar="",
        help="scale of interval between slices",
        default=15,
    )
    optional.add_argument(
        "-c",
        "--color",
        action="store_true",
        help="display registered image in color scale",
        default=False,
    )
    optional.add_argument(
        "-cr",
        ...
        type=int,
        metavar=("min", "max"),
        help="display registered image in colour scale within range min to max",
        default=(None, None),
    )
    optional.add_argument(
        "-o", "--out", type=str, metavar="", help="output image filename"
    )
...
def generate_tile_image(
    input_img, output_img, axis, slices, image_type, seg_img, color_scale, preprocdir
):
    ...
    mosaic_slicer.inputs.pad_or_crop = (
        "[ "
        + width_padding
        + "x "
        + height_padding
    ...
    if image_type == "reg" and color_scale:
        ...
        mosaic_slicer.inputs.rgb_image = os.path.join(
            preprocdir, image_type + "_rgb.nii.gz"
        )
    ...
    for i in range(len(slices)):
        draw.text(
            (5 + i * offset, tiled_image.height - 20),
            ORIENTATION_DICT[axis] + "=" + str(slices[i]),
            (255, 255, 255),
        )
...
def generate_pngs(
    ...
    if get_orient(fixed_img) != get_orient(reg_img):
        raise Exception(
            "Both the registration and the fixed image have different orientations"
        )

    if fixed_img.shape != reg_img.shape:
        raise Exception(
            "Both the registration and the fixed image have different voxel dimensions ({}, and {})".format(
                reg_img.shape, fixed_img.shape
            )
        )
    ...
    if seg_img and get_orient(seg_img) != get_orient(reg_img):
        raise Exception(
            "The segmentation mask's orientation is different than the others"
        )

    if slices * scale >= min(fixed_img.shape):
        raise Exception(
            "The slice and/or scale inputs are too large, exceed the dimensions of the registration and fixed images"
        )
    ...
    # generate 6 images
    for img_type, img in [
        ("fixed", preprocdir + "/fixed_canonical.nii.gz"),
        ("reg", preprocdir + "/reg_canonical.nii.gz"),
    ]:
        ...
        if color_scale and img_type == "reg":
            if minimum == None and maximum == None:
                command = (
                    "ConvertScalarImageToRGB 3 "
                    + img
                    + " "
                    + preprocdir
                    + "/"
                    + img_type
                    + "_rgb.nii.gz none hot"
                )
                os.system(command)
            else:
                ...
                converter.inputs.output_image = os.path.join(
                    preprocdir, img_type + "_rgb.nii.gz"
                )
        ...
        for axis in range(0, 3):
            ...
            output_file = os.path.join(
                preprocdir, "{}_{}_{}.png".format(prefix, img_type, axis)
            )
...
def combine_png(out_dir, prefix):
    ...
    fixed_images = glob.glob(
        os.path.join(out_dir, "svg_process", "{}_fixed_*.png".format(prefix))
    )
    fixed_images.sort()
    reg_images = glob.glob(
        os.path.join(out_dir, "svg_process", "{}_reg_*.png".format(prefix))
    )
    reg_images.sort()

    if not fixed_images or not reg_images:
        raise Exception(
            "Intermediate files are missing. You may be missing either a registration file or the fixed image"
        )
    ...
    fixed_image.save(
        os.path.join(
            out_dir, "svg_process", "{}_combined_fixed_image.png".format(prefix)
        )
    )
    ...
    reg_image.save(
        os.path.join(out_dir, "svg_process", "{}_combined_reg_image.png".format(prefix))
    )
...
def compile_svg(out_dir, out_file, prefix):
    ...
    """
    fixed_png = glob.glob(
        os.path.join(out_dir, "**/{}_combined_fixed_image.png".format(prefix))
    )
    reg_png = glob.glob(
        os.path.join(out_dir, "**/{}_combined_reg_image.png".format(prefix))
    )
...
def main(args):
    ...
    generate_pngs(
        fixed, reg, prefix, seg, color, minimum, maximum, out_dir, slices, scale
    )
