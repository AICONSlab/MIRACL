...
segpath = path + "/../segmentation_play/" ;
...
if (!File.exists(outmed)) {
	...
	// Create median image
	// run("3D Fast Filters","filter=Median radius_x_pix=2 radius_y_pix=2 radius_z_pix=2 Nb_cpus=24");
...
if (!File.exists(outmax)) {
	...
	// Find 3D Maxima image to create markers for Watershed
  	// run("3D Fast Filters","filter=MaximumLocal radius_x_pix=2 radius_y_pix=2 radius_z_pix=2 Nb_cpus=24");  	
