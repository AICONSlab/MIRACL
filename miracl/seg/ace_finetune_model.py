...
from monai.metrics import ConfusionMatrixMetric, DiceMetric, HausdorffDistanceMetric
...
def parsefn():
    ...
    parser.add_argument(
        "--train-images",
        help="train images directory should contain .tif or .tiff raw images",
        type=str,
        required=True,
    )
    parser.add_argument(
        "--train-labels",
        help="train ground truth images directory should contain .tif or .tiff raw images",
        type=str,
        required=True,
    )
    parser.add_argument(
        "--val-images",
        help="validation images directory should contain .tif or .tiff raw images",
        type=str,
        required=True,
    )
    parser.add_argument(
        "--val-labels",
        help="validation ground truth images directory should contain .tif or .tiff raw images",
        type=str,
        required=True,
    )
...
class RandomSaltPepper(Transform):
    def __call__(self, image_dict):
        ...
        image_dict["image"] = torch.tensor(
            random_noise(image_dict["image"], mode="s&p", amount=0.001, clip=True)
        )
        return image_dict
        ...
        [
            ...
            OneOf(
                transforms=[
                    ...
                    RandomSaltPepper(),
                    RandAdjustContrastd(
                        keys=["image"], prob=data_aug_prob, gamma=(0.5, 4)
                    ),
                    ...
                    RandHistogramShiftd(
                        keys=["image"], num_control_points=10, prob=data_aug_prob
                    ),
                ]
            ),
...
def configure_optimizers(
    ...
    if loss_name == "dicece":
        ...
        weight = torch.tensor(
            [1 / (cfg["loss"].get("dicece_weights", 1)), 1],
            dtype=torch.float,
            device=device,
        )
        ...
        loss_function = TverskyLoss(
            to_onehot_y=True, softmax=True, alpha=tversky_alpha, beta=tversky_beta
        )
...
def get_metrics() -> Tuple[DiceMetric, HausdorffDistanceMetric, ConfusionMatrixMetric]:
    ...
    metrics = ConfusionMatrixMetric(
        include_background=False, metric_name=["sensitivity", "precision", "f1 score"]
    )
...
def main(args):
    ...
    assert (
        images_train_path.exists()
    ), f"train images path {images_train_path} does not exist"
    assert (
        images_segs_train_path.exists()
    ), f"train labels path {images_segs_train_path} does not exist"
    assert images_val_path.exists(), f"val images path {images_val_path} does not exist"
    assert (
        images_segs_val_path.exists()
    ), f"val labels path {images_segs_val_path} does not exist"

    data_dicts_train, data_dicts_val = load_images(
        images_train_path, images_segs_train_path, images_val_path, images_segs_val_path
    )
    ...
    epoch_loss_values, dice_metric_values, hd_metric_values, metrics_values = train(
        model,
        model_name,
        cfg,
        train_loader,
    ...
    with (root_dir / "results.pickle").open("wb") as f:
        pickle.dump(
            [epoch_loss_values, dice_metric_values, hd_metric_values, metrics_values], f
        )
