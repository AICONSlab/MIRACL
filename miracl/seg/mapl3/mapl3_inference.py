...
this is the inference mode code for the 3D segmentation model
The code loads the trained model and performs inference on the test dataset
The code is written in a way that can be used for both single and multiple GPUs

Main Inputs:
- config: path of the config file used during training to define the model
- output_path: if passed it saves the results
- model_path: path of the trained model
- input_dir: path to input directory containing patches
- gpu_index: gpu index to be used; if you wanna use all the available gpus pass all; default it uses index 0
- binarization_threshold: threshold to binarize the model probabilty map; between 0-1
- save_prob_map_flag: a flag to whether save the prob map
- metadata: path to metadata JSON file (optional); it should be either original one or after classification label is added
- tissue_percentage_threshold: threshold between 0-100 to filter empty patches; if this is not passed, patches will be filtered based on classification label (required if metadata is provided)

Output:
- The code saves the output (binary and probability output (optional) as .tif files readable by Fiji
...
from miracl.system.utilfns.utilfns_cli_parser import str2bool, none_or_float
...
my_parser.add_argument(
    ...
    help="threshold between 0-100 to filter empty patches",
    required=False,
    default=None,
    type=none_or_float,
)
...
# load the data
def create_datadicts(
    input_file_path, metadata_path=None, percentage_brain_patch_skip=None
):
    # -------------------------------------------------------
    ...
    if metadata_path is not None:
        if percentage_brain_patch_skip is None:
            # Load metadata
            ...
            metadata_dict = {
                patch["filename"]: patch["classification_label"] for patch in metadata
            }

            # Filter non-empty images (classification_label > 0)
            images_non_empty = [
                os.path.join(input_file_path, image)
                for image in file_names_paths
                if metadata_dict.get(image, 0) > 0  # Use .get() to handle missing files
            ]
            ...
            images_empty = [
                os.path.join(input_file_path, image)
                for image in file_names_paths
                if metadata_dict.get(image, 0) <= 0  # Opposite condition
            ]
            images_empty.sort()

            # save output for empty patches
            # save_empty_out(output_dir, images_empty, save_prob_map_flag)

            print(
                f"metadata file is selected and percentage tissue thr is not selected, so patches will be filtered based on the classification label \n",
                f"the following files will be processed: \n",
            ...
            data_dicts = [{"image": image_name} for image_name in images_non_empty]
        ...
        elif percentage_brain_patch_skip is not None:
            # Load metadata
            with open(os.path.join(metadata_path)) as f:
                metadata = json.load(f)["patches"]
            ...
            images_non_empty = [
                ...
                > percentage_brain_patch_skip  # Use .get() to handle missing files
            ]
            images_non_empty.sort()
            ...
            print(
                f"metadata file is selected and percentage tissue thr is selected, so patches will be filtered based on the thr \n",
                f"the following files will be processed: \n",
                f"Number of empty patches: {len(images_empty)} \n",
                f"Number of non-empty patches: {len(images_non_empty)}",
            )
        ...
        data_dicts = [{"image": image_name} for image_name in file_names_paths]
        print("Data dicts are ready!")
        print(f"Some samples from data dicts: {data_dicts[:3]}")
        print(
            f"In total we have {len(data_dicts)} data dicts that the model will be deployed on"
...
def save_empty_out(root_dir, img_list_name, save_prob=False):
    for img in img_list_name:
        D, H, W = (tifffile.memmap(img)).shape

        name = os.basename(img)

        out = np.zeros((D, H, W), dtype=np.bool_)
        tifffile.imwrite(
            os.path.join(root_dir, "out", name),
            out,
            metadata={
                "DimensionOrder": "ZYX",
                "SizeC": 1,
        ...
        if save_prob:
            ...
            tifffile.imwrite(
                os.path.join(root_dir, "out_prob", name),
                out,
                metadata={
                    "DimensionOrder": "ZYX",
                    "SizeC": 1,
...
def image_saver(root_dir, name, outputs=None, thr=0.5, save_prob=False):
    post_pred = Compose(
        [
            ...
            AsDiscrete(to_onehot=outputs.shape[1]),
        ]
    )

    # only for the first channel (axons)
    outputs = torch.nn.functional.softmax(outputs, dim=1)[:, 1:2, ...]
...
def run_inference_on_gpu(
    ...
    with torch.no_grad():
        for i, data in enumerate(dataloader):
            ...
            print(f"Processing {data['img_file_name'][0]} on {device}")
...
def main(args):
    ...
    # create the output director(ies)
    output_dir = Path(args["output_path"]) / "inference"

    print("\n Running inference with the following settings:")
    print(f"  Patches folder path:    {input_file_path}")
    print(f"  Model path:             {model_path}")
    print(f"  GPU index:              {gpu_index}")
    print(f"  Binarization threshold: {binarization_threshold}")
    print(f"  Save probability map:   {save_prob_map_flag}")
    print(f"  Tissue % threshold:     {percentage_brain_patch_skip}")
    ...
    if not isExist:
        os.mkdir(output_dir)
        output_dir_out = os.path.join(output_dir, "out")
        if not os.path.exists(output_dir_out):
            os.mkdir(output_dir_out)

        if save_prob_map_flag:
            output_dir_out_prob = os.path.join(output_dir, "out_prob")
            if not os.path.exists(output_dir_out_prob):
                os.mkdir(output_dir_out_prob)

    # -------------------------------------------------------
    # load the data
    # -------------------------------------------------------
    data_dicts = create_datadicts(
        input_file_path, metadata_path, percentage_brain_patch_skip
    )
    # -------------------------------------------------------
