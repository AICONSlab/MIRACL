...
def parse_inputs(parser: argparse.ArgumentParser, args: list) -> th.Tuple:
            ...
            min_check(
                ...
                "".join(
                    (
                        "-",
                        tag
                        + " start_slice must not be a negative number. Current value: {}".format(
                            axis[0]
                        ),
                    )
                ),
            ...
            min_check(
                ...
                "".join(
                    (
                        "-",
                        tag
                        + " interval must not less than 1. Current value: {}".format(
                            axis[1]
                        ),
                    )
                ),
            ...
            min_check(
                ...
                "".join(
                    (
                        "-",
                        tag
                        + " number_of_slices  must not be less than 1. Current value: {}".format(
                            axis[2]
                        ),
                    )
                ),
            ...
            min_check(
                ...
                "".join(
                    (
                        "-",
                        tag
                        + " number_of_rows x number_of_columns is less than number_of_slices. row x col value: {} x {} = {}. Number_of_slices {}".format(
    ...
    if figure_dim == None and len(row) > 0:
        ...
        min_check(
            ...
            "".join(
                (
                    "Axis Rows and dpi, Height Resolution exceeds maximum. Rows: {}. DPI: {}. (Largest value of either Max Height or Total Rows) x DPI = {} x {} = {}. Cannot exceed 65535. Note: Max Height is capped at 60".format(
        ...
        min_check(
            ...
            "".join(
                (
                    "Axis Columns and dpi, Width Resolution exceeds maximum. Cols: {}. DPI: {}. (Largest value of either Max Width or Max Col) x DPI = {} x {} = {}. Cannot exceed 65535. Note: Max Width is capped at 60".format(
    ...
    if figure_dim != None:
        min_check(
            ...
            "".join(
                (
                    "-f/figure_dim and -dpi, Height resolution exceeds maximum. Height: {}. DPI: {}. Height x DPI = {}. Cannot exceed 65535".format(
        ...
        min_check(
            ...
            "".join(
                (
                    "-f/figure_dim and -dpi, Width resolution exceeds maximum. Width: {}. DPI: {}. Width x DPI = {}. Cannot exceed 65535".format(
...
def slice_display(atlas, sagittal, coronal, axial, x, y, z, n_slices):
    """slicing parameters"""
    ...
    if (
        ...
        for i in range(3):
            spacing = img_shape[i] / (n_slices + 1)
            cut_coords.append([round((j + 1) * spacing) for j in range(n_slices)])
        cut_len = 3
...
def figure_setup(
    cut_coords, cut_len, sagittal, coronal, axial, figure_dim, dpi, n_slices
):
    """plot configurations"""
    ...
    if len(cols) > 0:
        ...
        for i in range(cut_len):
            ...
            ## AA CHANGES
            ax.append(subfigs[i][0].subplots(1, n_slices, sharey=True))
...
def main(args):
    ...
    if hemi == "combined":
        ...
        brain_template = os.path.join(
            atlasdir,
            f"template/average_template_{vox}um.nii.gz",  # TODO
        )
    elif hemi == "split":
        mask = os.path.join(
    ...
    ### AA CHANGES START
    # multiply brain mask and img
    brain_mask_binary = brain_mask > 0
    from scipy.ndimage import binary_erosion

    brain_mask_binary = binary_erosion(brain_mask_binary, iterations=3)
    img1 = np.multiply(brain_mask_binary.astype("uint8"), img1)
    img1[img1 < 0] = 0
    ...
    cut_len, cut_coords = slice_display(
        mask, sagittal, coronal, axial, x, y, z, n_slices
    )
