...
from building_blocks import DoubleConv, ResNetBlock, create_decoders, create_encoders
from transformer_block import Block, Mlp
...
class ImageToPatch(nn.Module):
    ...
    def forward(self, x):
        if not self.is3d:
            ...
            x = torch.unflatten(x, 1, (-1, self.patch_size[0], self.patch_size[1], self.patch_size[2]))
            return x
        else:
            patch_size = self.patch_size
            img_size = self.img_size
            temp1, temp2, temp3 = img_size[0] // patch_size[0], img_size[1] // patch_size[1], img_size[2] // patch_size[2]
            x = einops.rearrange(x, '1 c (b1 h) (b2 w) (b3 d) -> (b1 b2 b3) c h w d', b1=temp1, b2=temp2, b3=temp3)
            return x
...
class PatchToImage(nn.Module):
    ...
    def forward(self, x):
        ...
        x = einops.rearrange(x, ' (b1 b2 b3) c h w d -> 1 c (b1 h) (b2 w) (b3 d)', b1=temp1, b2=temp2, b3=temp3)
        return x
...
class AbstractUNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, basic_module, f_maps=64, layer_order='gcr',
                 img_size = (512, 512, 512), crop_size=(128,128,128), dropout_prob=0.2,
                 num_groups=8, num_levels=4, is_segmentation=True, conv_kernel_size=3, pool_kernel_size=2, conv_padding=1, is3d=True, drop_path=0.1,
                 attention_sum=True, attention_weights=True, patch_wise_attn=True, patch_wise_attn_down=True, patch_wise_attn_down_factor=2, patch_wise_adaptive_thr=True, patch_wise_PE_3D=True,
                 encoder_depth_patch=5, atten_dim_patch=512, atten_depth_patch=8, num_heads_patch=8, mlp_ratio_patch=1., qkv_bias_patch=True, 
                 qk_scale_patch=1, drop_rate_patch=0., attn_drop_rate_patch=0., attn_drop_path_patch=0.1, norm_layer_patch=nn.LayerNorm, 
                 voxel_wise_attn=True, encoder_depth_voxel=5, atten_dim_voxel=512, atten_depth_voxel=8, num_heads_voxel=8, mlp_ratio_voxel=1., qkv_bias_voxel=True,
                 qk_scale_voxel=1, drop_rate_voxel=0., attn_drop_rate_voxel=0., attn_drop_path_voxel=0.1, norm_layer_voxel=nn.LayerNorm,
                 deep_supervision=0):
        ...
        self.encoders = create_encoders(in_channels, f_maps, basic_module, conv_kernel_size, conv_padding, layer_order,
                                        num_groups, pool_kernel_size, is3d, dropout_prob, drop_path)

        # create decoder path
        self.decoders = create_decoders(f_maps, basic_module, conv_kernel_size, conv_padding, layer_order, num_groups,
                                        is3d, dropout_prob, drop_path)
        ...
        self.is3d = is3d  # Store as instance attribute
        if self.is3d:
        ...
        if voxel_wise_attn:
            self.attention_blocks_voxel = nn.ModuleList([
                Block(dim=atten_dim_voxel, num_heads=num_heads_voxel, mlp_ratio=mlp_ratio_voxel, qkv_bias=qkv_bias_voxel, qk_scale=qk_scale_voxel,
                    drop=drop_rate_voxel, attn_drop=attn_drop_rate_voxel, drop_path=attn_drop_path_voxel, norm_layer=norm_layer_voxel)
            ...
            self.pos_embed_voxel = nn.Parameter(torch.zeros(1, n_seq_length, c_channel_size))
        ...
        if patch_wise_attn:
            self.attention_blocks_patch = nn.ModuleList([
                Block(dim=atten_dim_patch, num_heads=num_heads_patch, mlp_ratio=mlp_ratio_patch, qkv_bias=qkv_bias_patch, qk_scale=qk_scale_patch,
                    drop=drop_rate_patch, attn_drop=attn_drop_rate_patch, drop_path=attn_drop_path_patch, norm_layer=norm_layer_patch)
            ...
            if patch_wise_PE_3D:
                ...
                self.patch_wise_PE_inds = torch.zeros(patch_per_dim, patch_per_dim, patch_per_dim, 3).long()
                ...
                self.patch_wise_PE_x_params = nn.Parameter(torch.randn(patch_per_dim, pos_embed_dim))
                nn.init.trunc_normal_(self.patch_wise_PE_x_params)
                self.patch_wise_PE_y_params = nn.Parameter(torch.randn(patch_per_dim, pos_embed_dim))
                nn.init.trunc_normal_(self.patch_wise_PE_y_params)
                self.patch_wise_PE_z_params = nn.Parameter(torch.randn(patch_per_dim, atten_dim_patch - 2 * pos_embed_dim))
                nn.init.trunc_normal_(self.patch_wise_PE_z_params)
            else:
                self.pos_embed_patch = nn.Parameter(torch.zeros(1, num_patches, atten_dim_patch))
                
            self.pos_drop_patch = nn.Dropout(p=drop_rate_patch)

            if patch_wise_attn_down:
                self.patch_wise_attn_down_conv = nn.Conv3d(f_maps[encoder_depth_patch - 1] , int(f_maps[encoder_depth_patch - 1] / patch_wise_attn_down_factor), 1)
                self.patch_wise_attn_up_conv = nn.Conv3d(int(f_maps[encoder_depth_patch - 1]  / patch_wise_attn_down_factor), f_maps[encoder_depth_patch - 1] , 1)

            if patch_wise_adaptive_thr:

                self.Mlp = Mlp(in_features=atten_dim_patch,
        ...
        for i in range(deep_supervision):
            self.deep_supervision_module.append(nn.Conv3d(f_maps[i+1],out_channels,1))
    ...
    def voxel_wise_transformer(self, x):
        ...
        """applies a voxel wise transformer blocks
            ...
            then in transformer_block.py it needs batch size, sequence length, embed dimension,
        ...
        if self.voxel_wise_attn:
            ...
            x = torch.permute(x, (0, 2, 1)) # now should be B x sequence length x embed dim
            x = x.view(output_shape[0], output_shape[1], output_shape[2],output_shape[3],output_shape[4])
    ...
    def patch_wise_transformer(self, x):
        ...
        if self.patch_wise_attn:
            ...
            x = torch.unsqueeze(x, 0) # should be (Batch, sequence length, patch embeb dimension)
            ...
            if self.patch_wise_PE_3D:
                ...
                flattened_inds = self.patch_wise_PE_inds.reshape(-1, 3).to(self.patch_wise_PE_x_params.device)
                x_vecs = torch.index_select(self.patch_wise_PE_x_params, dim=0, index=flattened_inds[:, 0]) 
                y_vecs = torch.index_select(self.patch_wise_PE_y_params, dim=0, index=flattened_inds[:, 1])
                z_vecs = torch.index_select(self.patch_wise_PE_z_params, dim=0, index=flattened_inds[:, 2])
                patch_wise_PE = torch.unsqueeze(torch.cat([x_vecs, y_vecs, z_vecs], dim = 1), 0)
            else:
    ...
    def forward(self, x):
        ...
        for i, encoder in enumerate(self.encoders):
            ...
            if not self.attention_sum:
                ...
                if (i == self.encoder_depth_voxel-1) and (i == self.encoder_depth_patch-1):
                    
                    x_voxel_transformer = self.voxel_wise_transformer(x)
                    x_patch_transformer, adaptive_thr_bias = self.patch_wise_transformer(x)

                    if self.attention_weights:
                        #------------------------
                        # combine voxel_wise and patch_wise if attention_weighing is enabled
                        #------------------------            
                        w = torch.cat([self.w_voxel_transformer, self.w_patch_transformer])
                        w_voxel, w_patch = w.softmax(dim=0)
                        w_voxel = torch.unsqueeze(w_voxel, 0)
                        w_patch = torch.unsqueeze(w_patch, 0)

                        x =  (w_voxel * x_voxel_transformer + w_patch * x_patch_transformer) + x # x is residual block
                    
                    # just add voxel wise and patch wise results
                    else: 

                        x =  x_voxel_transformer + x_patch_transformer + x # x is residual block
        ...
        if self.patch_wise_adaptive_thr and self.patch_wise_attn:
            x = x + adaptive_thr_bias.reshape(-1, adaptive_thr_bias.size(1), 1, 1, 1) 
        ...
        if self.training and self.deep_supervision_num > 0:
            ...
            deep_supervision_output = self.compute_deep_supervision_output(deep_supervision_output)
...
class UNet3D(AbstractUNet):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid=True, f_maps=64, layer_order='gcr',
                 img_size = (512, 512, 512), crop_size=(128,128,128), dropout_prob=0.2,
                 num_groups=8, num_levels=4, is_segmentation=True, conv_padding=1, drop_path=0.1, attention_weights=True, attention_sum=True, 
                 patch_wise_attn=True, patch_wise_attn_down=True, patch_wise_attn_down_factor=2, patch_wise_adaptive_thr=True, patch_wise_PE_3D=True,
                 encoder_depth_patch=5, atten_dim_patch=512, atten_depth_patch=8, num_heads_patch=8, mlp_ratio_patch=1., qkv_bias_patch=True, qk_scale_patch=1, 
                 drop_rate_patch=0., attn_drop_rate_patch=0., attn_drop_path_patch = 0.1, norm_layer_patch=nn.LayerNorm,
                 voxel_wise_attn=True, encoder_depth_voxel=5, atten_dim_voxel=512, atten_depth_voxel=8, num_heads_voxel=8, mlp_ratio_voxel=1., qkv_bias_voxel=True,
                 qk_scale_voxel=1, drop_rate_voxel=0., attn_drop_rate_voxel=0., attn_drop_path_voxel=0.1, norm_layer_voxel=nn.LayerNorm, **kwargs):
        super(UNet3D, self).__init__(in_channels=in_channels,
                                     ...
                                     patch_wise_attn=patch_wise_attn, patch_wise_attn_down=patch_wise_attn_down, patch_wise_attn_down_factor=patch_wise_attn_down_factor,
                                     patch_wise_adaptive_thr=patch_wise_adaptive_thr, patch_wise_PE_3D=patch_wise_PE_3D, encoder_depth_patch=encoder_depth_patch,
                                     atten_dim_patch=atten_dim_patch,atten_depth_patch=atten_depth_patch, num_heads_patch=num_heads_patch, 
                                     mlp_ratio_patch=mlp_ratio_patch, qkv_bias_patch=qkv_bias_patch, qk_scale_patch=qk_scale_patch, drop_rate_patch=drop_rate_patch, 
                                     attn_drop_rate_patch=attn_drop_rate_patch, attn_drop_path_patch=attn_drop_path_patch, norm_layer_patch=norm_layer_patch,
                                     voxel_wise_attn=voxel_wise_attn, encoder_depth_voxel=encoder_depth_voxel, atten_dim_voxel=atten_dim_voxel, atten_depth_voxel=atten_depth_voxel, num_heads_voxel=num_heads_voxel, 
                                     mlp_ratio_voxel=mlp_ratio_voxel, qkv_bias_voxel=qkv_bias_voxel, qk_scale_voxel=qk_scale_voxel, drop_rate_voxel=drop_rate_voxel, 
                                     attn_drop_rate_voxel=attn_drop_rate_voxel, attn_drop_path_voxel=attn_drop_path_voxel, norm_layer_voxel=norm_layer_voxel)
...
class ResidualUNet3D(AbstractUNet):
    ...
    """
    def __init__(self, in_channels, out_channels, final_sigmoid=True, f_maps=64, layer_order='gcr',
                 conv_kernel_size=3, pool_kernel_size=2, is3d=True,
                 img_size = (512, 512, 512), crop_size=(128,128,128),dropout_prob=0.2,
                 num_groups=8, num_levels=5, is_segmentation=True, conv_padding=1, drop_path=0.1, attention_weights = True, attention_sum=True,
                 patch_wise_attn=True, patch_wise_attn_down=True, patch_wise_attn_down_factor=2, patch_wise_adaptive_thr=True, patch_wise_PE_3D=True,
                 encoder_depth_patch=5, atten_dim_patch=512, atten_depth_patch=8,
                 num_heads_patch=8, mlp_ratio_patch=1., qkv_bias_patch=True, qk_scale_patch=1, 
                 drop_rate_patch=0., attn_drop_rate_patch=0., attn_drop_path_patch = 0.1, norm_layer_patch=nn.LayerNorm,
                 voxel_wise_attn=True, encoder_depth_voxel=5, atten_dim_voxel=512, atten_depth_voxel=8, num_heads_voxel=8, mlp_ratio_voxel=1., qkv_bias_voxel=True,
                 qk_scale_voxel=1, drop_rate_voxel=0., attn_drop_rate_voxel=0., attn_drop_path_voxel=0.1, norm_layer_voxel=nn.LayerNorm,
                 deep_supervision = 0, **kwargs):
        super(ResidualUNet3D, self).__init__(in_channels=in_channels,
                                             ...
                                             attention_weights=attention_weights,
                                             attention_sum=attention_sum,
                                             patch_wise_attn=patch_wise_attn, atten_dim_patch=atten_dim_patch, atten_depth_patch=atten_depth_patch, num_heads_patch=num_heads_patch, 
                                             patch_wise_adaptive_thr=patch_wise_adaptive_thr, patch_wise_PE_3D=patch_wise_PE_3D, encoder_depth_patch= encoder_depth_patch, 
                                             mlp_ratio_patch=mlp_ratio_patch, qkv_bias_patch=qkv_bias_patch, qk_scale_patch=qk_scale_patch, drop_rate_patch=drop_rate_patch, 
                                             attn_drop_rate_patch=attn_drop_rate_patch, patch_wise_attn_down=patch_wise_attn_down, patch_wise_attn_down_factor=patch_wise_attn_down_factor, 
                                             attn_drop_path_patch=attn_drop_path_patch, norm_layer_patch=norm_layer_patch,
                                             voxel_wise_attn=voxel_wise_attn, encoder_depth_voxel=encoder_depth_voxel, atten_dim_voxel=atten_dim_voxel, atten_depth_voxel=atten_depth_voxel, num_heads_voxel=num_heads_voxel, 
                                             mlp_ratio_voxel=mlp_ratio_voxel, qkv_bias_voxel=qkv_bias_voxel, qk_scale_voxel=qk_scale_voxel, drop_rate_voxel=drop_rate_voxel, 
                                             attn_drop_rate_voxel=attn_drop_rate_voxel, attn_drop_path_voxel=attn_drop_path_voxel, norm_layer_voxel=norm_layer_voxel,
