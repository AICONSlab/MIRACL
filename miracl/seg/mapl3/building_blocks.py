...
# modified from https://github.com/wolny/pytorch-3dunet/blob/master/pytorch3dunet/unet3d/buildingblocks.py
# by Ahmadreza Attarpour, a.attarpour@mail.utoronto.ca, Grace Yu gracefengqing.yu@mail.utoronto.ca
...
def create_conv(in_channels, out_channels, kernel_size, order, num_groups, padding, is3d, dropout_prob):
    """
    ...
    Args:
        ...
        order (string): order of things, e.g.
            ...
            'cl' -> conv + LeakyReLU
            'ce' -> conv + ELU
            'bcrd' -> batchnorm + conv + ReLU + dropout
    ...
    for i, char in enumerate(order):
        ...
        # added dropout layer
            ...
            modules.append(('dropout', nn.Dropout(dropout_prob)))
            
        else:
            raise ValueError(f"Unsupported layer type '{char}'. MUST be one of ['b', 'g', 'r', 'l', 'e', 'c','d','p']")
...
class SingleConv(nn.Sequential):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, order='gcr', num_groups=8, padding=1, is3d=True, dropout_prob=0.2):
        super(SingleConv, self).__init__()

        for name, module in create_conv(in_channels, out_channels, kernel_size, order, num_groups, padding, is3d, dropout_prob):
    ...
    def __init__(self, in_channels, out_channels, encoder, kernel_size=3, order='gcr', num_groups=8, padding=1,
        ...
        self.add_module('SingleConv1',
                        SingleConv(conv1_in_channels, conv1_out_channels, kernel_size, order, num_groups,
                                   padding=padding, is3d=is3d, dropout_prob=dropout_prob))
        # conv2
...
class ResNetBlock(nn.Module):
    ...
    it uses three buliding blocks:
    1) is only a conv layer without pooling and stride
    2) is based on the order we choose in the config file
    3) is based one the order we choose in the config without activation function
    if residual applied, the output of 1 will be connected to 3


    """

    def __init__(self, in_channels, out_channels, kernel_size=3, order='cge', num_groups=8, is3d=True, dropout_prob=0.2, drop_path=0.1, **kwargs):
        ...
        self.conv2 = SingleConv(out_channels, out_channels, kernel_size=kernel_size, order=order, num_groups=num_groups,
                                is3d=is3d, dropout_prob=dropout_prob)
        # remove non-linearity from the 3rd convolution since it's going to be applied after adding the residual
        n_order = order
        for c in 'relp':
            n_order = n_order.replace(c, '')
        self.conv3 = SingleConv(out_channels, out_channels, kernel_size=kernel_size, order=n_order,
                                num_groups=num_groups, is3d=is3d, dropout_prob=dropout_prob)

        # create non-linearity separately
        # Need to change the PReLU here
        if 'l' in order:
...
class Encoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, apply_pooling=True,
                 ...
                 num_groups=8, padding=1, is3d=True,dropout_prob=0.2, drop_path=0.1):
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, scale_factor=(2, 2, 2), basic_module=DoubleConv,
                 conv_layer_order='gcr', num_groups=8, mode='nearest', padding=1, upsample=True, is3d=True, dropout_prob=0., drop_path=0.1):
...
def create_encoders(in_channels, f_maps, basic_module, conv_kernel_size, conv_padding, layer_order, num_groups,
                    pool_kernel_size, is3d, dropout_prob=0.2, drop_path=0.1):
...
def create_decoders(f_maps, basic_module, conv_kernel_size, conv_padding, layer_order, num_groups, is3d, dropout_prob=0.2, drop_path=0.1):
