...
if (!File.exists(outenhance)) {
	...
	run("Enhance Contrast...", "saturated=5 normalize process_all use");
...
if (!File.exists(outfil)) {
	...
	run("Shape Filter", "area=7-30 circularity=0-15 elongation=0-0.7 draw_holes black_background exclude_on_edges stack");
