...
# get version
function getversion() {
  local ver=$(cat $MIRACL_HOME/version.txt)
  printf "\n MIRACL pipeline v. %s\n" "${ver}"
}
...
function usage() {
  ...
  cat <<usage
	...
	1) Registers in-vivo or ex-vivo MRI data to Allen Reference mouse brain Atlas or Fischer/Waxholm rat brain Atlases
	2) Warps Atlas annotations to the MRI space

        The GUI for this module is currently not available for v${ver} but will be added back in a later version.

	For command-line / scripting

	Usage: miracl reg mri_ants -i [ input invivo or exvivo MRI nii ] -o [ orient code ] -a [ atlas ] -m [ hemi mirror ] -v [ labels vox ] -l [ input labels ] -b [ olfactory bulb ] -s [ skull strip ] -n [ no orient needed ]

    Example: miracl reg mri_ants -i inv_mri.nii.gz -o RSP -m combined -v 25
    ...
    optional arguments:
        o.  orient code (default: RSP)
            to orient nifti from original orientation to "standard/Allen/Fischer/Waxholm" orientation
        a.  atlas (default: allen)
            use 'allen' for mouse models and 'fischer' or 'waxholm' atlas for rat models
            accepted inputs are: <allen>, <fischer> or <waxholm>
...
# else
#
#   if [ "${atl}" != "allen" ] && [ "${atl}" != "fischer" ] && [ "${atl}" != "waxholm" ]; then
#
#     printf "ERROR: < -a => (atl) > only takes as inputs: allen, fischer or waxholm"
#     exit 1
#
#   fi
#
...
if [ "${atl}" == "allen" ]; then
  ...
  printf "ERROR: < -a => (atl) > Only accepted atlases (inputs) are: 'allen', 'fischer', or 'waxholm'"
  exit 1
fi
...
function initmratlasreg() {
  ...
  ifdsntexistrun "${initform}" "Initializing registration ..." \
    antsAffineInitializer 3 "${mrlnk}" "${atlasref}" "${initform}" "${deg}" "${radfrac}" "${useprincax}" "${localiter}"

  # Warp Atlas
  ifdsntexistrun "${initatlas}" "initializing ${atl^} template" antsApplyTransforms -i "${atlasref}" -r "${mrlnk}" -t "${initform}" -o "${initatlas}"

}

#---------------------------

# 2b) Register to Atlas Function
...
function regmratlas() {
  ...
  # Perform ANTs registration between MRI and Atlas

  ifdsntexistrun "${antsatlas}" "Registering MRI data to ${atl^} atlas ... this will take a while" \
    ants_miracl_mr -d 3 -f "${mrprereg}" -m "${atlas}" -o "${regdir}"/"${atl}"_mr_ants -t "${trans}" -p "${prec}" -n "${thrds}" -s "${spldist}" -r "${rad}" | tee "${regdir}"/ants_reg.log

}

#---------------------------

# 3) Warp Atlas labels to original MRI Function
...
# 4) Warp MRI to Atlas space Function

function warpinmratlas() {
  ...
  ifdsntexistrun "${regorgmr}" "Applying ants deformation to input MRI" \
    antsApplyTransforms -r "${atlashres}" -i "${orthresmr}" -n Bspline -t [ "${initform}", 1 ] [ "${antsaff}", 1 ] "${antsinvwarp}" -o "${regorgmr}" --float
...
function main() {
  ...
  if [ "${atl}" == "allen" ]; then
    ...
    if [[ "${bulb}" == 0 ]]; then
      atlasref=${atlasdir}/ara/template/average_template_50um_OBmasked.nii.gz

    elif [[ "${bulb}" == 1 ]]; then
      atlasref=${atlasdir}/ara/template/average_template_50um.nii.gz
    fi

  elif [ "${atl}" == "fischer" ]; then
    # Fischer atlas template
    atlasref=${atlasdir}/fischer/template/Fischer344_template_stripped.nii.gz
  ...
  # Out Atlas
  antsatlas="${regdir}"/"${atl}"_mr_antsWarped.nii.gz

  #	regmratlas ${orghdmr} ${initatlas} ${trans} ${spldist} ${rad} ${prec} ${thrds} ${antsatlas}
  regmratlas "${mrlnk}" "${atlasref}" "${trans}" "${spldist}" "${rad}" "${prec}" "${thrds}" "${antsatlas}"

  #---------------------------

  # 3) Warp Atlas labels to MRI

  # Tforms
  antswarp=${regdir}/${atl}_mr_ants1Warp.nii.gz
  antsaff=${regdir}/${atl}_mr_ants0GenericAffine.mat
  ...
  warpatlaslbls ${mrlnk} ${lbls} ${antswarp} ${antsaff} ${wrplbls}

  #---------------------------

  # 4) Warp input MRI to Atlas

  #	# ort hres mr
  #	ortinmr=${regdir}/org_mr_ort.nii.gz
  #
  #	# hres Atlas
  #	atlasres=${atlasdir}/ara/template/average_template_10um.nii.gz
  #
  #	# ants inv warp
  #	antsinvwarp=${regdir}/${atl}_mr_ants1InverseWarp.nii.gz
  #
  # out warp hres mr
  regmratlas=${regdirfinal}/mr_${atl}_ants.nii.gz
  antsinvmr=${regdir}/${atl}_mr_antsInverseWarped.nii.gz
  #
