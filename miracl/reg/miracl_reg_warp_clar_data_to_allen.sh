...
if [[ "$#" -gt 1 ]]; then
  ...
  printf "\n  Warping will be run with the following settings:\n"
  printf "  r: Input clarity registration dir: %s\n" "${regdir}"
  printf "  i: Input downsampled CLARITY nii to warp: %s\n" "${inimg}"
  printf "  o: File with orientation to standard code: %s\n" "${ortfile}"
  printf "  s: Segmentation channel: %s\n" "${channel}"
  printf "  v: Voxel resolution: %s\n" "${voxres}"
  printf "  l: input Allen labels: %s\n" "${lbls}"
