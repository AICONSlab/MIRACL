...
def run_clar_allen(parser, args):
    ...
    if len(sys.argv) == 3 and sys.argv[-2] == 'reg' and sys.argv[-1] == 'clar_allen':
        ...
        if args['help']:
            ...
            bash_args = '-i %s -r %s -o %s -m %s -v %s -l %s -f %s -p %s -a %s -w %s -b %s -s %s -t %s' \
                        % (args['in_nii'],   # -i
                           args['reg_out'],  # -r
                           args['ort'],      # -o
                           args['hemi'],     # -m
...
def get_parser():
    ...
    parser_clar_allen.add_argument('-t', '--percentile_thr', metavar='', default=0,
                                      help="percentile threshold for intensity correction")
