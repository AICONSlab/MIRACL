...
def parsefn():
    if len(sys.argv) >= 3 and sys.argv[-2] == "reg" and sys.argv[-1] == "check":
        ...
        parser.add_argument(
            ...
            "--viz",
            type=str,
            help="Visualization software",
        )
        parser.add_argument(
            "-s",
            "--space",
            type=str,
            help="Registration Space",
        )
        parser.add_argument(
            "-m",
            "--hemi",
            type=str,
            help="Hemisphere (split or combined)",
        )
...
def parse_inputs(parser, args):
    ...
    if sys.argv[-2] == "reg" and sys.argv[-1] == "check":
        ...
        indirstr = subprocess.check_output(
            ...
            % (
                miracl_home,
                "Final registration directory",
                "Registration Space (def = clarity)",
                "Hemisphere (def = combined)",
...
def main(args):
    ...
    if viz == "itk":
        ...
        if space == "clarity":
            ...
            subprocess.check_call(
                "itksnap -g %s/clar_allen_space.nii.gz -o $allen%d -s $lbls%d -l %s"
                % (indir, res, res, snaplut),
                shell=True,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
            )
