...
comb=$statdir/${id}_comb_lbls_stats.csv

pushd $statdir

# do I need to delete any rows!
# sed '7,12d' ${id}_clar_feats_allen_lbls.csv > feat.tmp
...
for met in FA MD T1 T2 CT; do
	...
	if [[ "$met" == "CT" ]]; then
		...
		awk '{print $2}' ${id}_exv_${met}_allen_lbls.csv > ${met}.tmp

	else

		awk '{print $2}' ${id}_inv_${met}_allen_lbls.csv > ${met}.tmp
